...
const Mouse:FC<IMouse> = ({className, serviceCard, textCenter, icon = true}) => {
    ...
    return (
        <div className="flex flex-col gap-y-16 ">
            {icon && (
                <div className={`hidden md:flex flex-col justify-center items-center ${className}` }>
                    <img src={mouse} alt="" />
                    <img src={line} alt="" />
                </div>
            )}    
            {serviceCard?.description !== undefined && <div className="flex flex-col justify-center items-center gap-y-4">
                ...
                <p className={`${textCenter ? "" : "xl:ml-36"} text-base leading-5 text-white`}>{serviceCard.description}</p>
            </div>}
        </div>
        )
}
